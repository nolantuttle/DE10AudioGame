...
if rising_edge(i2c_clk_50) then
	...
	-- For our i2c serial clock (SCL), we are doing 50% of the ~200kHz clock with SCL high
	if(clk_prs < 125) then
		i2c_scl_clk <= '1';
	else
		i2c_scl_clk <= '0';
	...
	-- Halfway through when SCL is high, we should set ACK
	if(clk_prs = 62) then
		ack_en <= '1';
	else
		ack_en <= '0';
	...
	-- Set clock for data when SCL is low
	if(clk_prs = 187) then 
		clk_en <= '1';
	else
		clk_en <= '0';
...
if rising_edge(i2c_clk_50) then
    ...
    case state is
        ...
        when IDLE =>
            i2c_sda <= '1';
            i2c_busy <= '0';
            -- Clear i2c_done only when starting a new transaction
        ...
        when STOP =>
            i2c_clk_en <= '0';
            i2c_sda <= '0';
            i2c_busy <= '0';  -- Clear busy flag when transaction completes
            state <= IDLE;
            i2c_done <= '1';
